 ...
 */
export const Forkit = ({ primary, backgroundColor, size, label, ...props }) => {
  var STATE_CLOSED = 0,
    STATE_DETACHED = 1,
    STATE_OPENED = 2,
    TAG_HEIGHT = 30,
  ...
  let closedX = TAG_WIDTH * 0.4;
  let closedY = TAG_HEIGHT * 0.5;
  // Resting position of the ribbon when curtain is opened
  let openedX = TAG_WIDTH * 0.4;
  let openedY = TAG_HEIGHT;
  let velocity = 0;
  let rotation = 45;
  ...
  let dragTime = 0;
  let dragY = 0;
  let anchorA = new Point(closedX, closedY);
  let anchorB = new Point(closedX, closedY);
  let mouse = new Point();
  ...
  function update() {
    ...
    if (state === STATE_OPENED) {
      curtainTargetY = Math.min(
        curtainTargetY + (window.innerHeight - curtainTargetY) * 0.2,
        window.innerHeight
      );
    } else {
    ...
    if (dragging || state === STATE_DETACHED) {
      ...
      var offsetX = Math.max(
        (mouse.x - containerOffsetX - closedX) * 0.2,
        -MAX_STRAIN
      );
      ...
      var angle = Math.min(
        130,
        Math.max(50, (Math.atan2(dy, dx) * 180) / Math.PI)
      );
  ...
  function render() {
    if (dom.curtain) {
      dom.curtain.style.top =
        -100 +
        Math.min((curtainCurrentY / window.innerHeight) * 100, 100) +
        "%";
    }

    dom.ribbon.style[prefix("transform")] = transform(0, curtainCurrentY, 0);
    dom.ribbonTag.style[prefix("transform")] = transform(
      anchorB.x,
      anchorB.y,
      rotation
    );
    ...
    dom.ribbonString.style[prefix("transform")] = transform(
      anchorA.x,
      0,
      angle
    );
  ...
  function prefix(property, el) {
    ...
    for (var i = 0, len = VENDORS.length; i < len; i++) {
      ...
      if (
        typeof (el || document.body).style[vendor + propertyUC] !== "undefined"
      ) {
  ...
  return (
    <a onMouseOver={initialize}
      className="forkit"
      data-text="Top Secret"
      data-text-detached="Drag down >"
      href="https://github.com/hakimel/forkit.js"
    >
      <img
